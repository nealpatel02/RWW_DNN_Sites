...
- A shared DLL for Rand Store is copied into the `References` folder for other extensions to reference independently.  
...
**Rand Content Templates**  
There are a couple of template projects that may or may not need to be updated to be treated differently in either deployment and/or source control.  
...
- ETLScript: This folder contains scripts that were used for the IMAGINiT website rebranding. It could eventually be removed from the solution after the IMAGINiT rebrand is launched into production and is determined to be stable.  
- Install: This folder has resources that were used for the IMAGINiT website rebranding. It could eventually be removed from the solution after the IMAGINiT rebrand is launched into production and is determined to be stable.  
...
- RazorScripts: These files are OpenContent templates that will eventually need to be moved into a central template project for all template files (including those that are already in template projects in the main solution).  
...
- Templates: Contains project(s) that help to deploy content templates for OpenContent and other template-driven solutions.  
...
Themes are intentionally not in the standard Visual Studio solution.  Instead, you should navigate to the main folder for the respective theme since it is build using [nvQuickTheme](https://nvquicktheme.com/) and includes a number of command lines to help theme developers have a quicker and easier time using the project and intended tools.  
...
If you're adding/editing references that come from **Evoq** (or from the References folder), DO NOT use the Visual Studio IDE to do this.  It will result in long-term management issues for the solution. Instead, you should view any of the `.csproj` or `.vbproj` files in a separate text editor to see how to properly add a reference.  It's not possible to add it correctly in the Visual Studio IDE.  

In order to know which is which, the first step is to see and/or add the known references to the `References` folder.  Then, update the `SolutionReferences.targets` file in the `Build` folder to point to this new path with a new `MSBuild` variable, if necessary.  Finally, open the respective project file(s) in your preferred editor and add the references using the same pattern you see for other existing local references.  

#### General Instructions  
When necessary, first add the new references to the correct references folder/path, and then update the `SolutionReferences.targets` file in the `Build` folder.  
